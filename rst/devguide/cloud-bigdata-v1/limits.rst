...
Rate limits are specified in both a human-readable wildcard URI and a machine-processable regular expression. The regular expression boundary matcher ``^`` takes effect after the root URI path. For example, the regular expression ``^/v1.0/clusters`` would match the last portion of the following URI: https://dfw.bigdata.api.rackspacecloud.com/v1.0/clusters.
...
Rate limits are applied in order relative to the verb, going from least to most specific. For example, although the threshold for issuing a **POST** request to ``/v1.0/*`` is 2 per minute, you cannot issue a **POST** request to ``/v1.0/*`` more than 50 times within a single day.
